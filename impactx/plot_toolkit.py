...
def visualize__main( refp=False, stat=False, poincare=False, post=False, \
    ...
    if ( refp or stat ):
        df_stat = itk.get__beamStats( statFile=files["stat"], refpFile=files["refp"] )

    # ------------------------------------------------- #
    # --- [2] ref_particle plot                     --- #
    # ------------------------------------------------- #
    if ( refp ):
        plot__beamStats( df=df_stat, plot_conf=plot_conf, kind="refp" )

    # ------------------------------------------------- #
    # --- [3] reduced beam characteristics plot     --- #
    # ------------------------------------------------- #
    if ( stat ):
        plot__beamStats( df=df_stat, plot_conf=plot_conf, kind="stat" )
    ...
    if ( post ):
        ...
        plot__beamStats( df=df_post, plot_conf=plot_conf, kind="post" )
...
def plot__beamStats( df=None, plot_conf=None, kind="refp" ):
...
def plot__trajectories( plot_conf=None, pids=None ):
