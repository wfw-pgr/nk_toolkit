...
def optimize__quadFromEnvelope( inpFile   ="dat/matching.json",
    ...
    def _runOptimizer():
        ...
        bounds       = [ ( variable["min"], variable["max"] ) for variable in variables ]
        ...
        # --- [5] return                                --- #
        # ------------------------------------------------- #
        return( result, history, bestVector, bestEvaluation )
    ...
    if ( runMode == "optimize" ):
        ...
        result, history, bestVector, bestEvaluation = _runOptimizer()
    ...
    if ( result is not None ):
        ...
        print( " objective :: {:.8e} -> {:.8e}".format( float( history[0]["objective"] ),
               bestValue ) )
