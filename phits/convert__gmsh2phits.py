import contextlib, io, os, sys, json5
import meshio
import numpy as np
...
def convert__gmsh2phits( mshFile="msh/model.msh", bdfFile="msh/model.bdf", \
    ...
    if ( config is None ):
        if ( configFile is None ):
            raise FileNotFoundError(
                "[convert__gmsh2phits.py] config=?, configFile=? [ERROR]" )
        else:
    ...
    GRID_fmt       = "".join( [ "%8s", "%16d", "%16d", "%16.9e", "%16.9e", "%s",
                                "%8s", "%16.9e", "%16d" ] )
    GRID_Data      = np.concatenate( [ GRID_cards[:,None], GRID_IDs[:,None],
                                       coord_IDs1[:,None], \
